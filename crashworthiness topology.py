...
try:
    ...
    print("Error: lasso library not found. Please install it using 'pip install lasso-python'.")
    exit()
try:
...
def run_dyna_safe(k_file, work_dir, log_file=None, timeout_sec=None):
    ...
    try:
        ...
        print(f"   DYNA execution error: {e}")
        return False, 0.0
...
class SRIShellAssembler:
    def __init__(self, nodes_dict, elements_list, E, nu, t, fixed_nodes):
        ...
        print("      Initializing CSR memory mapping for sparse matrix assembly...")
        ...
        print("      CSR memory mapping initialized.")
...
class RealTimeVisualizer:
    ...
    def save_animation(self, save_path, fps=5):
        if self.frames:
            print(f"      Saving animation (FPS={fps})...")
            try:
                imageio.mimsave(save_path, self.frames, duration=int(1000/fps), loop=0)
                print(f"      Animation saved: {save_path}")
            except Exception as e: print(f"      Animation saving failed: {e}")
class SIMP_Optimizer:
    def __init__(self, assembler, target_vols_dict, element_pids, penalty, r_filter_ratio, move_limit, triple_loop_prefix=""):
        ...
        if PYPARDISO_AVAILABLE:
            ...
            print(f"      PyPardiso solver enabled with symbolic factorization caching.")
    ...
    def run_optimization(self, current_x, current_beta, macro_step, global_time_logs, max_iter=20):
        ...
        for k in range(max_iter):
             ...
             for case in self.esl_cases:
                 ...
                 print(f"      [Timing] 组装: {t_asm_end-t_asm_start:.3f}s | 求解: {t_sol_end-t_asm_end:.3f}s | 敏度: {t_sens_end-t_sol_end:.3f}s")
             ...
             print(f"      ⏱️ [Timing] Current SIMP iteration (Local {k+1}) total time: {local_time:.3f}s")
             ...
             if (change < 0.01 or obj_change < SIMP_OBJ_TOLERANCE) and k > 5:
                 print(f"      Internal SIMP iteration converged. (Change: {change:.4f}, Obj Error: {obj_change:.2e})")
                 break
...
def run_diesl_double_loop(target_domains, work_dir, run_k_file, nodes, elements, element_ids, element_pids, prefix_name, global_time_logs):
    ...
    for macro_step in range(MAX_MACRO_ITERATIONS):
        ...
        print(f"\n   [DiESL Phase 1 {macro_step+1}/{MAX_MACRO_ITERATIONS} | Beta={current_beta:.1f}] macro-scale impact analysis and equivalent load reconstruction...")
        ...
        print(f"        Crash response: [{', '.join([f'S{i+1}={f:.1f}N' for i, f in enumerate(F_stages_phase1)])}], D={D_max_phase1:.2f}mm | EA={EA_phase1:.2f}")
        print(f"      [Timing] LS-DYNA nonlinear analysis time: {dyna_time:.2f} 秒")
        d3_loader = D3PlotLoader(os.path.join(work_dir, "d3plot"))
        try: d3_loader.load_file()
        except: raise RuntimeError("Critical error: LS-DYNA did not generate d3plot. Please check the log file.")
        ...
        if prev_U_vec is not None and curr_U_vec is not None:
            ...
            print(f"      Global displacement field variation: {change*100:.2f}%")
            if change < MACRO_TOLERANCE:
                print("      Macroscopic physical field converged. DiESL terminated.")
                break
        ...
        print(f"      [Timing] Internal SIMP iteration time: {simp_time:.2f} 秒")
...
def extract_beso_voids(d3plot_path, target_vols, element_ids, element_pids, centers, vols, r_min, history_sens=None, impactor_node_id=132725, impact_dir=1, enable_rebound=True, sym_groups=None):
    print(f"      Reading D3PLOT to extract element internal energy at the selected deformation state...")
    ...
    if 'element_shell_internal_energy' not in d3_loader.d3.arrays.keys(): raise KeyError("Element shell internal energy data were not found. Please check the ENGFLG setting in the K file.")
    ...
    print(f"      Applying BESO spatial filtering and history averaging...")
    tree = KDTree(centers)
...
def main():
    ...
    print("\n" + ""*35)
    print(" Start two-stage structural optimization framework (DiESL + BESO hard-kill scheme)")
    print(" Phase 1: Equivalent static load based topology optimization")
    print(f" Bridge: applying {BRIDGE_VOL_RATIO:.0%} solid/void truncation based on the prescribed volume ratio")
    print(" Phase 2: BESO-based crashworthiness refinement using spatially filtered sensitivity")
    print(""*35)
    try:
        ...
        print(f"     Initial full design-domain mass {full_design_mass:.4e} t")
    except Exception as e:
        print(f"Input reading error: {e}")
        return
    current_target_domains, _ = extract_matsum_centroids(matsum_path, DESIGN_PIDS)
    print(f"\n{'='*60}\n [Phase 1] Starting DiESL-based macro-scale topology optimization\n{'='*60}")
    ...
    print("\n   DiESL macro-scale topology optimization completed.")
    print("\n   Generating visualization K file for the final DiESL result with separated part IDs...")
    visual_void_ids = set()
    for pid in DESIGN_PIDS:
        pid_indices = np.where(element_pids == pid)[0]
